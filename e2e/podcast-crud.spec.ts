...
test.describe("Podcast CRUD Operations", () => {
  ...
  test("should edit podcast information", async ({ page }) => {
    ...
    const editButton = page.locator('button:has-text("Edit"), [data-testid="edit-podcast"]').first();
    ...
    if (await editButton.isVisible({ timeout: 5000 })) {
      ...
      const nameInput = page.locator('input[name="name"], input[placeholder*="name"], input[value*="Podcast"]').first();
  ...
  test("should handle podcast settings and branding", async ({ page }) => {
    ...
    await expect(page.locator('h1, h2, [data-testid="settings-title"]')).toBeVisible({ timeout: 10000 });
    
    // Look for branding or color customization options
    const brandingSection = page.locator('text=Brand, text=Color, text=Theme, [data-testid="branding"]').first();
  ...
  test("should manage podcast members", async ({ page }) => {
    ...
    const membersSection = page.locator('text=Members, text=Team, text=Users, [data-testid="members"]').first();
    ...
    if (await membersSection.isVisible({ timeout: 5000 })) {
      ...
      const currentUserIndicator = page.locator('text=Owner, text=Admin, [data-testid="current-user"]').first();
      ...
      const addMemberButton = page.locator('button:has-text("Add Member"), button:has-text("Invite"), [data-testid="add-member"]').first();
  ...
  test("should delete a podcast", async ({ page }) => {
    ...
    const dangerZone = page.locator('text=Danger, text=Delete, [data-testid="danger-zone"]').first();
    
    if (await dangerZone.isVisible({ timeout: 5000 })) {
      await dangerZone.scrollIntoViewIfNeeded();
      
      // Find delete button
      const deleteButton = page.locator('button:has-text("Delete"), [data-testid="delete-podcast"]').first();
      
      if (await deleteButton.isVisible()) {
        await deleteButton.click();
        
        // Handle confirmation dialog
        const confirmInput = page.locator('input[placeholder*="confirm"], input[placeholder*="delete"]').first();
