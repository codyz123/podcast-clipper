...
test.describe("Episode Workflow", () => {
  ...
  test("should create a new episode", async ({ page }) => {
    ...
    const createButton = page.locator('button:has-text("Create"), button:has-text("New Episode"), [data-testid="create-episode"]').first();
    ...
    if (await createButton.isVisible({ timeout: 5000 })) {
      ...
      const nameInput = page.locator('input[name="name"], input[placeholder*="episode"], input[placeholder*="title"]').first();
  ...
  test("should import audio and transcribe", async ({ page }) => {
    ...
    const importArea = page.locator('[data-testid="import-area"], .upload-zone, input[type="file"]').first();
    ...
    if (await importArea.isVisible({ timeout: 5000 })) {
      ...
      if (await fileInput.isVisible()) {
        ...
        const progressIndicator = page.locator('.progress, [data-testid="upload-progress"]').first();
        ...
        const transcribeButton = page.locator('button:has-text("Transcribe"), [data-testid="transcribe-button"]').first();
        ...
        if (await transcribeButton.isVisible({ timeout: 5000 })) {
          ...
          const transcriptionProgress = page.locator('.transcription-progress, [data-testid="transcribe-progress"]').first();
  ...
  test("should edit transcript and manage speakers", async ({ page }) => {
    ...
    const transcriptEditor = page.locator('[data-testid="transcript-editor"], .transcript-container').first();
    ...
    if (await transcriptEditor.isVisible({ timeout: 10000 })) {
      ...
      if (await speakerSection.isVisible()) {
        // Add a speaker
        const addSpeakerButton = page.locator('button:has-text("Add Speaker"), [data-testid="add-speaker"]').first();
        
        if (await addSpeakerButton.isVisible()) {
          await addSpeakerButton.click();
          
          const speakerNameInput = page.locator('input[placeholder*="speaker"], input[placeholder*="name"]').first();
      ...
      if (await editableTranscript.isVisible()) {
        ...
        // Save changes
        const saveTranscriptButton = page.locator('button:has-text("Save"), [data-testid="save-transcript"]').first();
  ...
  test("should create and edit video clips", async ({ page }) => {
    ...
    if (await clipInterface.isVisible({ timeout: 10000 })) {
      ...
      const createClipButton = page.locator('button:has-text("Create Clip"), button:has-text("Add Clip"), [data-testid="create-clip"]').first();
      
      if (await createClipButton.isVisible()) {
        await createClipButton.click();
        
        // Set clip details
        const clipNameInput = page.locator('input[name="title"], input[placeholder*="title"], input[placeholder*="name"]').first();
        ...
        if (await clipNameInput.isVisible()) {
          ...
          // Save clip
          const saveClipButton = page.locator('button:has-text("Save"), button[type="submit"]').first();
      ...
      const suggestClipsButton = page.locator('button:has-text("Suggest"), button:has-text("AI"), [data-testid="suggest-clips"]').first();
      ...
      if (await suggestClipsButton.isVisible()) {
        ...
        const suggestedClips = page.locator('[data-testid="suggested-clip"], .clip-suggestion').first();
        
        if (await suggestedClips.isVisible({ timeout: 10000 })) {
          // Accept a suggestion
          const acceptButton = page.locator('button:has-text("Accept"), button:has-text("Use"), [data-testid="accept-clip"]').first();
  ...
  test("should customize video settings and render", async ({ page }) => {
    ...
    const videoEditor = page.locator('[data-testid="video-editor"], .video-editor-container').first();
    
    if (await videoEditor.isVisible({ timeout: 10000 })) {
      // Test format selection
      const formatSelector = page.locator('[data-testid="format-selector"], select[name="format"]').first();
      ...
      const backgroundSelector = page.locator('[data-testid="background-selector"], .background-options').first();
      ...
      const subtitleOptions = page.locator('[data-testid="subtitle-options"], .subtitle-config').first();
      ...
      const renderButton = page.locator('button:has-text("Render"), [data-testid="render-video"]').first();
      
      if (await renderButton.isVisible()) {
        await renderButton.click();
        
        // Wait for render to start
        const renderProgress = page.locator('[data-testid="render-progress"], .render-status').first();
        ...
        if (await renderProgress.isVisible({ timeout: 10000 })) {
          ...
          const renderComplete = page.locator('text=Complete, text=Finished, [data-testid="render-complete"]').first();
  ...
  test("should configure publishing and upload to platforms", async ({ page }) => {
    ...
    if (await publishPanel.isVisible({ timeout: 10000 })) {
      ...
      const platformConnections = page.locator('[data-testid="platform-connections"], .oauth-connections').first();
      ...
      if (await postConfig.isVisible()) {
        ...
        // Set hashtags
        const hashtagInput = page.locator('input[name="hashtags"], input[placeholder*="hashtag"]').first();
      ...
      const scheduleOption = page.locator('[data-testid="schedule-post"], .schedule-settings').first();
  ...
  test("should delete an episode", async ({ page }) => {
    ...
    if (await episodeMenu.isVisible({ timeout: 5000 })) {
      ...
      const deleteButton = page.locator('button:has-text("Delete"), [data-testid="delete-episode"]').first();
